...
class GiphyService {
  ...
  static Future<List<GiphyGif>> searchGifs(String query, {int limit = 20}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return results.map((json) => GiphyGif.fromJson(json)).toList();
      } else {
  ...
  /// Buscar GIFs em destaque
  /// 
  /// [limit] - Número de resultados (padrão: 20)
  static Future<List<GiphyGif>> getTrendingGifs({int limit = 20}) async {
    try {
      final url = Uri.parse(
